...
std::string StateCacheDB::lookup(h256 const& _h) const
{
    ...
    if (it != m_main.end())
    {
        if (!m_enforceRefs || it->second.second > 0)
            ...
            //XXXX cwarn << "Lookup required for value with refcount == 0. This is probably a critical trie issue" << _h;
            std::cout << "Lookup required for value with refcount == 0. This is probably a critical trie issue" << _h;
    }
    return std::string();
}
...
{
	//std::cout << "statecachedb::insert" << std::endl;
#if DEV_GUARDED_DB
    WriteGuard l(x_this);
#endif
    auto it = m_main.find(_h);
