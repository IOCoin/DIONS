...
class NRelay
{
    ...
    virtual void* exch(unsigned char*) {
        return 0;
    }

    virtual void* trans(unsigned char*, int) {
        return 0;
    }
    virtual void* sen() {
        return 0;
...
public:
    ...
    void correlate(int c) {
        this->corr_ = c;
    }

    int g_sample(NodeEx& ex) {
        return ex.react(this->rel_);
    }

    int g_resolve(NodeEx& ex) {
        return ex.reg(this->rel_);
    }
    void g_base(__BASE__& g) {
        this->g_ = g;
    }
    int  indexC() {
        return this->indexC_;
...
public:
    ...
    virtual void accept(NRelay& rel) {
        this->relay_ = rel;
    }
    virtual int lmap() {
        return this->lMap_;
    }
    virtual void lmap(int lm) {
        this->lMap_ = lm;
    }
private:
    int lMap_;
