...
void SettingsPage::setModel(WalletModel *model)
{
    ...
    if(model)
    {
        ...
        settingsView->setModel(transactionProxyModel);
        settingsView->setAlternatingRowColors(true);
        settingsView->setSelectionBehavior(QAbstractItemView::SelectRows);
        settingsView->setSelectionMode(QAbstractItemView::ExtendedSelection);
        settingsView->setSortingEnabled(true);
        settingsView->sortByColumn(TransactionTableModel::Date, Qt::DescendingOrder);
        settingsView->verticalHeader()->hide();

        settingsView->horizontalHeader()->resizeSection(
                TransactionTableModel::Status, 23);
        settingsView->horizontalHeader()->resizeSection(
                TransactionTableModel::Date, 120);
        settingsView->horizontalHeader()->resizeSection(
                TransactionTableModel::Type, 120);
        settingsView->horizontalHeader()->setResizeMode(
