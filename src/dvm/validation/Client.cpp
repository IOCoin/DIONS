...
std::ostream& dev::dvm::operator<<(std::ostream& _out, ActivityReport const& _r)
{
    ...
    fs::path const& _snapshotDownloadPath, WithExisting _forceAction, u256 _networkId)
{
    ...
    // create dvm capability only if we're not downloading the snapshot
    if (_snapshotDownloadPath.empty())
    {
        auto dvmCapability = make_shared<dvmCapability>(
            _extNet.capabilityHost(), bc(), m_stateDB, m_tq, m_bq, _networkId);
        _extNet.registerCapability(dvmCapability);
        m_host = dvmCapability;
    }
...
{
    // TODO: more precise check on whdvmer the txs match.
...
TransactionSkeleton Client::populateTransactionWithDefaults(TransactionSkeleton const& _t) const
{
    ...
    // value used by gdvm and testrpc.
