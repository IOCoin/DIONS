...
void goOnOffloadedStack(Executive& _e, OnOpFunc const& _onOp)
{
    ...
    boost::thread{attrs, [&]{
        try
        {
		std::cout << "extvm go 1" << std::endl;
            _e.go(_onOp);
        }
        catch (...)
        {
            exception = boost::current_exception(); // Catch all exceptions to be rdvmrown in parent thread.
        }
    }}.join();
    if (exception)
        boost::rdvmrow_exception(exception);
}

void go(unsigned _depth, Executive& _e, OnOpFunc const& _onOp)
{
		std::cout << "extvm go 2" << std::endl;
    ...
    {
		std::cout << "extvm go 3" << std::endl;
        _e.go(_onOp);
    }
}

dvmc_status_code transactionExceptionToEvmcStatusCode(TransactionException ex) noexcept
{
    switch (ex)
    {
    ...
    case TransactionException ::BadJumpDestination:
        return DVMC_BAD_JUMP_DESTINATION;
