...
#include <libdvmereum/CodeSizeCache.h>
#include <libdvm/ExtVMFace.h>
#include <array>
...
/**
 * Model of an dvm state, essentially a facade for the trie.
 *
 ...
 * The changelog is managed by savepoint(), rollback() and commit() mdvmods.
 */
class State
{
    friend class ExtVM;
...
public:
    ...
    /// which uses it. If you have no preexisting database then set BaseState to somdvming other
    ...
    /// @param _commitBehaviour whdvmer or not to remove empty accounts during commit.
