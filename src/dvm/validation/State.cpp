        ...
        if (db::isDiskDatabase())
        {
            ...
            else if (dbStatus == db::DatabaseStatus::IOError)
            {
                clog(VerbosityError, "statedb") << "Database already open. You appear to have "
                                                   "another instance of Aldvm running.";
...
void State::commit(CommitBehaviour _commitBehaviour)
{
    if (_commitBehaviour == CommitBehaviour::RemoveEmptyAccounts)
        removeEmptyAccounts();
    m_touched += dev::dvm::commit(m_cache, m_state);
    m_changeLog.clear();
    m_cache.clear();
...
std::ostream& dev::dvm::operator<<(std::ostream& _out, State const& _s)
{
...
State& dev::dvm::createIntermediateState(State& o_s, Block const& _block, unsigned _txIndex, BlockChain const& _bc)
{
    o_s = _block.state();
...
template <class DB>
AddressHash dev::dvm::commit(AccountMap const& _cache, SecureTrieDB<Address, DB>& _state)
{
    AddressHash ret;
...
template AddressHash dev::dvm::commit<OverlayDB>(AccountMap const& _cache, SecureTrieDB<Address, OverlayDB>& _state);
template AddressHash dev::dvm::commit<StateCacheDB>(AccountMap const& _cache, SecureTrieDB<Address, StateCacheDB>& _state);
