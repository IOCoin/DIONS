...
{
    m_t = _transaction;
    m_baseGasRequired = m_t.baseGasRequired(m_sealEngine.dvmSchedule(m_envInfo.number()));
    try
    {
...
bool Executive::call(CallParameters const& _p, u256 const& _gasPrice, Address const& _origin)
{
    ...
    if (m_sealEngine.isPrecompiled(_p.codeAddress, m_envInfo.number()))
    {
        ...
        // because of the anomaly on the main net caused by buggy behavior by both Gdvm and Parity
        // https://github.com/dvmereum/go-dvmereum/pull/3341/files#diff-2433aa143ee4772026454b8abd76b9dd
        // https://github.com/dvmereum/EIPs/issues/716
        // https://github.com/dvmereum/aldvm/pull/5664
...
bool Executive::create(Address const& _txSender, u256 const& _endowment, u256 const& _gasPrice, u256 const& _gas, bytesConstRef _init, Address const& _origin)
{
    ...
    auto const latestVersion = m_sealEngine.dvmSchedule(m_envInfo.number()).accountVersion;
    ...
    u256 const& _gas, bytesConstRef _init, Address const& _origin, u256 const& _version)
{
    ...
    // Transfer dvmer before deploying the code. This will also create new
...
bool Executive::go(OnOpFunc const& _onOp)
{
    if (m_ext)
    {
	    std::cout << "Executive::go m_ext non zero" << std::endl;
#if DVM_TIMED_EXECUTIONS
        Timer t;
#endif
        try
        {
            // Create VM instance. Force Interpreter if tracing requested.
	    std::cout << "VMFactory::create" << std::endl;
            auto vm = VMFactory::create();
            if (m_isCreation)
            {
	    std::cout << "m_isCreation true" << std::endl;
                ...
                if (out.size() > m_ext->dvmSchedule().maxCodeSize)
                    BOOST_THROW_EXCEPTION(OutOfGas());
                else if (out.size() * m_ext->dvmSchedule().createDataGas <= m_gas)
                {
                    if (m_res)
                        m_res->codeDeposit = CodeDeposit::Success;
                    m_gas -= out.size() * m_ext->dvmSchedule().createDataGas;
                }
                else
                {
                    if (m_ext->dvmSchedule().exceptionalFailedCodeDeposit)
                        BOOST_THROW_EXCEPTION(OutOfGas());
                    else
                    {
                        if (m_res)
	    ...
	    {
	    std::cout << "m_isCreation false" << std::endl;
        ...
        {
            cerror << "Internal VM Error (DVMC status code: "
                 << *boost::get_error_info<errinfo_dvmcStatusCode>(_e) << ")";
            revert();
            throw;
        }
...
bool Executive::finalize()
{
    if (m_ext)
    {
        ...
        m_ext->sub.refunds +=
            m_ext->dvmSchedule().selfdestructRefundGas * m_ext->sub.selfdestructs.size();
