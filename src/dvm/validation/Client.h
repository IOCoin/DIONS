...
/**
 * @brief Main API hub for interfacing with dvm.
 */
...
public:
    ...
    /// Get the object representing the current state of dvm.
    dev::dvm::Block postState() const { ReadGuard l(x_postSeal); return m_postSeal; }
    ...
    std::unique_ptr<StateImporterFace> createStateImporter() { return dev::dvm::createStateImporter(m_stateDB); }
    std::unique_ptr<BlockChainImporterFace> createBlockChainImporter() { return dev::dvm::createBlockChainImporter(m_bc); }
...
protected:
    ...
    std::weak_ptr<dvmCapability> m_host;
