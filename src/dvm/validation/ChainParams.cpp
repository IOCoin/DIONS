...
using namespace dev;
using namespace dvm;
using namespace dvm::validation;
  ...
  : ChainParams(_configJson)
{
    ...
    // create DVM schedule
    DVMSchedule const& lastForkSchedule = forkScheduleForBlockNumber(lastForkBlock);
    lastForkWithAdditionalEIPsSchedule = DVMSchedule{lastForkSchedule, _additionalEIPs};
}
...
void ChainParams::loadGenesis(string const& _json, h256 const& _stateRoot)
{
    ...
    // magic code for handling dvmash stuff:
...
h256 ChainParams::calculateStateRoot(bool _force) const
{
    ...
    if (!stateRoot || _force)
    {
        ...
        dev::dvm::commit(genesisState, state);
