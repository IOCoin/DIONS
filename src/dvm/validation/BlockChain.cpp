...
#include <libdvmereum/DatabasePaths.h>
...
std::ostream& dev::dvm::operator<<(std::ostream& _out, BlockChain const& _bc)
{
...
db::Slice dev::dvm::toSlice(h256 const& _h, unsigned _sub)
{
...
db::Slice dev::dvm::toSlice(uint64_t _n, unsigned _sub)
{
#if ALL_COMPILERS_ARE_CPP11_COMPLIANT
    ...
    fs::path const& _path, std::function<void(unsigned, unsigned)> const& _progress)
{
    ...
    if (fs::exists(m_dbPaths->extrasTemporaryPath()))
    {
        ...
        LOG(m_loggerError) << "Please re-run Aldvm the --kill option to delete all databases. This "
    ...
    else
    {
        ...
        LOG(m_loggerError)
            << "Please re-run Aldvm with the --kill option to delete all databases. This will "
    ...
    VerifiedBlocks const& _blocks, OverlayDB const& _stateDB)
{
    ...
    for (VerifiedBlock const& block : _blocks)
    {
        do {
            ...
            catch (dev::dvm::AlreadyHaveBlock const&)
            {
                cwarn << "ODD: Import queue contains already imported block";
                continue;
            }
            catch (dev::dvm::UnknownParent const&)
            {
            ...
            catch (dev::dvm::FutureTime const&)
            {
                cwarn << "ODD: Import queue contains a block with future time.";
                this_thread::sleep_for(chrono::seconds(1));
                continue;
            }
            catch (dev::dvm::TransientError const&)
            {
...
void BlockChain::insert(VerifiedBlockRef _block, bytesConstRef _receipts, bool _mustBeNew)
{
    ...
    // togdvmer with an "ensureCachedWithUpdatableLock(l)" mdvmod.
...
{
    //@tidy This is a behemoth of a mdvmod - could do to be split into a few smaller ones.
...
vector<unsigned> BlockChain::withBlockBloom(LogBloom const& _b, unsigned _earliest, unsigned _latest, unsigned _level, unsigned _index) const
{
    ...
    for (unsigned o = obegin; o < oend; ++o)
        ...
        {
            // This level has somdvming like what we want.
            if (_level > 0)
...
Block BlockChain::genesisBlock(OverlayDB const& _db) const
{
    ...
    {
        ret.noteChain(*this);
        dev::dvm::commit(m_params.genesisState, ret.mutableState().m_state);        // bit horrible. maybe consider a better way of constructing it?
