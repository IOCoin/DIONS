...
#include <libdvmcore/SealEngine.h>
...
public:
    ...
    ExtVM(State& _s, EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, Address _myAddress,
      ...
      : ExtVMFace(_envInfo, _myAddress, _caller, _origin, _value, _gasPrice, _data, _code.toBytes(),
        ...
        m_dvmSchedule(initEvmSchedule(envInfo().number(), _version))
    {
    ...
    {
        if (dvmSchedule().emptinessIsNonexistence())
    ...
    DVMSchedule const& dvmSchedule() const final { return m_dvmSchedule; }
...
private:
    DVMSchedule const& initEvmSchedule(int64_t _blockNumber, u256 const& _version) const
    {
        ...
        DVMSchedule const& currentBlockSchedule = m_sealEngine.dvmSchedule(_blockNumber);
    ...
    DVMSchedule const& m_dvmSchedule;
};
