...
evmc_result execute(const VM& vm, ExecutionState& state, const CodeAnalysis& analysis) noexcept
{
    ...
    while (true)  // Guaranteed to terminate because padded code ends with STOP.
    {
        ...
        case OP_CREATE:
        {
            const auto status_code = create<DVMC_CREATE>(state);
            if (status_code != DVMC_SUCCESS)
            {
                state.status = status_code;
                goto exit;
        ...
        case OP_CALL:
        {
            const auto status_code = call<DVMC_CALL>(state);
            if (status_code != DVMC_SUCCESS)
            {
                state.status = status_code;
                goto exit;
        ...
        case OP_CALLCODE:
        {
            const auto status_code = call<DVMC_CALLCODE>(state);
            if (status_code != DVMC_SUCCESS)
            {
                state.status = status_code;
                goto exit;
        ...
        case OP_RETURN:
            op_return<DVMC_SUCCESS>(state);
            goto exit;
        case OP_DELEGATECALL:
        {
            const auto status_code = call<DVMC_DELEGATECALL>(state);
            if (status_code != DVMC_SUCCESS)
            {
                state.status = status_code;
                goto exit;
        ...
        case OP_STATICCALL:
        {
            const auto status_code = call<DVMC_CALL, true>(state);
            if (status_code != DVMC_SUCCESS)
            {
                state.status = status_code;
                goto exit;
        ...
        case OP_CREATE2:
        {
            const auto status_code = create<DVMC_CREATE2>(state);
            if (status_code != DVMC_SUCCESS)
            {
                state.status = status_code;
                goto exit;
        ...
        case OP_REVERT:
            op_return<DVMC_REVERT>(state);
            goto exit;
        case OP_INVALID:
            state.status = DVMC_INVALID_INSTRUCTION;
            goto exit;
...
exit:
    const auto gas_left =
        (state.status == DVMC_SUCCESS || state.status == DVMC_REVERT) ? state.gas_left : 0;
