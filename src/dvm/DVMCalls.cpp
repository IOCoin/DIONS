...
void DVM::copyDataToMemory(bytesConstRef _data, u256*_sp)
{
...
void DVM::throwOutOfGas()
{
    BOOST_THROW_EXCEPTION(OutOfGas());
}

void DVM::throwBadInstruction()
{
    BOOST_THROW_EXCEPTION(BadInstruction());
}

void DVM::throwBadJumpDestination()
{
    BOOST_THROW_EXCEPTION(BadJumpDestination());
}

void DVM::throwDisallowedStateChange()
{
...
void DVM::throwBadStack(unsigned _removed, unsigned _added)
{
...
void DVM::throwRevertInstruction(owning_bytes_ref&& _output)
{
...
void DVM::throwBufferOverrun(bigint const& _endOfAccess)
{
...
int64_t DVM::verifyJumpDest(u256 const& _dest, bool _throw)
{
    // check for overflow
...
:   bytesRef output;
    if (caseCallSetup(callParams.get(), output))
    {
        ...
        m_SPP[0] = result.status == DVMC_SUCCESS ? 1 : 0;
    }
    else
        m_SPP[0] = 0;
...
bool DVM::caseCallSetup(CallParameters *callParams, bytesRef& o_output)
{
