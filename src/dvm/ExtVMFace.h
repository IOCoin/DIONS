...
#include <libdvmcore/DVMSchedule.h>
#include <libdvmcore/LogEntry.h>
...
public:
    ...
    {}
    // Constructor with custom gasLimit - used in some synthetic scenarios like dvm_estimateGas RPC
    // mdvmod
    EnvInfo(BlockHeader const& _current, LastBlockHashesFace const& _lh, u256 const& _gasUsed,
...
///
/// @todo: Replace with dvmc_result in future.
struct CallResult
{
    dvmc_status_code status;
    owning_bytes_ref output;

    CallResult(dvmc_status_code status, owning_bytes_ref&& output)
...
struct CreateResult
{
    ...
    CreateResult(dvmc_status_code status, owning_bytes_ref&& output, h160 const& address)
...
public:
    ...
    ///
    /// @param beneficiary  The address of the account which will receive DVM
    ...
    /// Return the DVM gas-price schedule for this execution context.
    virtual DVMSchedule const& dvmSchedule() const { return DefaultSchedule; }
...
public:
    ...
    dvmc::bytes32 get_storage(const dvmc::address& _addr, const dvmc::bytes32& _key) const
        noexcept override;

    dvmc_storage_status set_storage(const dvmc::address& _addr, const dvmc::bytes32& _key,
        const dvmc::bytes32& _value) noexcept override;

    dvmc::uint256be get_balance(const dvmc::address& _addr) const noexcept override;

    size_t get_code_size(const dvmc::address& _addr) const noexcept override;

    dvmc::bytes32 get_code_hash(const dvmc::address& _addr) const noexcept override;

    size_t copy_code(const dvmc::address& _addr, size_t _codeOffset, uint8_t* _bufferData,
        size_t _bufferSize) const noexcept override;

    void selfdestruct(
        const dvmc::address& _addr, const dvmc::address& _beneficiary) noexcept override;

    dvmc::result call(const dvmc_message& _msg) noexcept override;

    dvmc_tx_context get_tx_context() const noexcept override;

    dvmc::bytes32 get_block_hash(int64_t _blockNumber) const noexcept override;

    void emit_log(const dvmc::address& _addr, const uint8_t* _data, size_t _dataSize,
        const dvmc::bytes32 _topics[], size_t _numTopics) noexcept override;

private:
    dvmc::result create(dvmc_message const& _msg) noexcept;
