...
struct BlockAnalysis
{
    ...
    {
        return {clamp<decltype(BlockInfo{}.track_cost)>(track_cost),
            clamp<decltype(BlockInfo{}.stack_req)>(stack_req),
            clamp<decltype(BlockInfo{}.stack_max_growth)>(stack_max_growth)};
    }
};

AdvancedCodeAnalysis analyze(dvmc_revision rev, bytes_view code) noexcept
{
    ...
    while (code_pos != code_end)
    {
        ...
        block.track_cost += opcode_info.track_cost;
        ...
        case OP_CREATE2:
        case OP_SSTORE:
            instr.arg.number = block.track_cost;
            break;
