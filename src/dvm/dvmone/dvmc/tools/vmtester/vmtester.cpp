...
int main(int argc, char* argv[])
{
    try
    {
        ...
        auto cli = cli_parser{"DVMC VM Tester", PROJECT_VERSION, {"MODULE"}};

        const auto error_code = cli.parse(argc, argv, std::cout, std::cerr);
        if (error_code <= 0)
            return error_code;

        const auto& dvmc_module = cli.arguments[0];
        std::cout << "Testing " << dvmc_module << "\n";

        dvmc_loader_error_code ec = DVMC_LOADER_UNSPECIFIED_ERROR;
        auto vm = dvmc::VM{dvmc_load_and_configure(dvmc_module.c_str(), &ec)};
