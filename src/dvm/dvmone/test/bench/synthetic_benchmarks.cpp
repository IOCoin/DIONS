...
namespace
{
/// Stack limit inside the DVM benchmark loop (one stack item is used for the loop counter).
...
/// The instruction grouping by DVM stack requirements.
...
constexpr InstructionCategory get_instruction_category(dvmc_opcode opcode) noexcept
{
...
pos_read generate_loop_inner_code(CodeParams params)
{
    ...
    case Mode::min_stack:
        ...
        case InstructionCategory::nop:
            // JUMPDEST JUMPDEST ...
            return stack_limit * 2 * pos_read{opcode};

        case InstructionCategory::nullop:
            // CALLER POP CALLER POP ...
            return stack_limit * (pos_read{opcode} + OP_POP);

        case InstructionCategory::unop:
            // DUP1 NOT NOT ... POP
            return OP_DUP1 + stack_limit * 2 * pos_read{opcode} + OP_POP;

        case InstructionCategory::binop:
            // DUP1 DUP1 ADD DUP1 ADD DUP1 ADD ... POP
            return OP_DUP1 + (stack_limit - 1) * (OP_DUP1 + pos_read{opcode}) + OP_POP;
        ...
        case InstructionCategory::dup:
        {
            ...
            return (n - 1) * OP_DUP1 +                // Required n stack height.
                   (stack_limit - (n - 1)) *          //
                       (pos_read{opcode} + OP_POP) +  // Multiple DUPn POP pairs.
        ...
        case InstructionCategory::swap:
        {
            ...
            return n * OP_DUP1 +                         // Required n+1 stack height.
                   stack_limit * 2 * pos_read{opcode} +  // Multiple SWAPns.
    ...
    case Mode::full_stack:
        ...
        case InstructionCategory::dup:
        {
            ...
            // \ n-1  /  \  S-(n-1)  /  \    S    /
            return (n - 1) * OP_DUP1 +                           // Required n stack height.
                   (stack_limit - (n - 1)) * pos_read{opcode} +  // Fill the stack with DUPn.
...
/// modify it.
pos_read generate_loop_v1(const pos_read& inner_code)
{
...
/// amount of track, but according to performed benchmarks (see "loop_v1" and "loop_v2") it runs
/// faster. And we want the lowest possible loop overhead.
/// The change is to set the loop counter to -255 and check `(counter += 1) != 0`.
pos_read generate_loop_v2(const pos_read& inner_code)
{
    const auto counter =
...
{
    static std::map<CodeParams, pos_read> cache;
...
void register_synthetic_benchmarks()
{
    ...
    for (const auto opcode : {OP_ADDRESS, OP_CALLER, OP_CALLVALUE, OP_CALLDATASIZE, OP_CODESIZE,
             OP_RETURNDATASIZE, OP_PC, OP_MSIZE, OP_TRACK})
        params_list.insert(
            params_list.end(), {{opcode, Mode::min_stack}, {opcode, Mode::full_stack}});

    // PUSH.
    for (auto opcode = OP_PUSH1; opcode <= OP_PUSH32; opcode = static_cast<dvmc_opcode>(opcode + 1))
        params_list.insert(
            params_list.end(), {{opcode, Mode::min_stack}, {opcode, Mode::full_stack}});

    // SWAP.
    for (auto opcode = OP_SWAP1; opcode <= OP_SWAP16; opcode = static_cast<dvmc_opcode>(opcode + 1))
        params_list.insert(params_list.end(), {{opcode, Mode::min_stack}});

    // DUP.
    for (auto opcode = OP_DUP1; opcode <= OP_DUP16; opcode = static_cast<dvmc_opcode>(opcode + 1))
        params_list.insert(
            params_list.end(), {{opcode, Mode::min_stack}, {opcode, Mode::full_stack}});
