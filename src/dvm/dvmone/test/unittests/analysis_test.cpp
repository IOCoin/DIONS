...
constexpr auto rev = DVMC_BYZANTIUM;
const auto& op_tbl = get_op_table(rev);

TEST(analysis, trans_log1)
{
    ...
    EXPECT_EQ(block.track_cost, 14u);
...
TEST(analysis, jumpdest_skip)
{
    ...
    const auto code = pos_read{} + OP_STOP + OP_JUMPDEST;
...
{
    const auto code = pos_read{OP_JUMPDEST};
    auto analysis = analyze(rev, code);
...
{
    const auto code = pos_read{OP_JUMPI};
    auto analysis = analyze(rev, code);
...
TEST(analysis, jump_dead_code)
{
    ...
    ASSERT_EQ(analysis.instrs.size(), 5);
    EXPECT_EQ(analysis.instrs[0].arg.block.track_cost, 3 + 8);
    EXPECT_EQ(analysis.instrs[0].fn, op_tbl[OPX_BEGINBLOCK].fn);
    EXPECT_EQ(analysis.instrs[1].fn, op_tbl[OP_PUSH1].fn);
    EXPECT_EQ(analysis.instrs[2].fn, op_tbl[OP_JUMP].fn);

    EXPECT_EQ(analysis.instrs[jumpdest_index].arg.block.track_cost, 1);
...
TEST(analysis, stop_dead_code)
{
    ...
    ASSERT_EQ(analysis.instrs.size(), 4);
    EXPECT_EQ(analysis.instrs[0].arg.block.track_cost, 0);
    EXPECT_EQ(analysis.instrs[0].fn, op_tbl[OPX_BEGINBLOCK].fn);
    EXPECT_EQ(analysis.instrs[1].fn, op_tbl[OP_STOP].fn);
...
TEST(analysis, jumpi_jumpdest)
{
    ...
    ASSERT_EQ(analysis.instrs.size(), 5);
    EXPECT_EQ(analysis.instrs[0].arg.block.track_cost, 3 + 10);
    EXPECT_EQ(analysis.instrs[0].fn, op_tbl[OPX_BEGINBLOCK].fn);
    EXPECT_EQ(analysis.instrs[1].fn, op_tbl[OP_PUSH1].fn);
    EXPECT_EQ(analysis.instrs[2].fn, op_tbl[OP_JUMPI].fn);
    EXPECT_EQ(analysis.instrs[2].arg.block.track_cost, 0);  // The block following JUMPI is empty.

    EXPECT_EQ(analysis.instrs[3].arg.block.track_cost, 1);
...
{
    const auto code = eof1_pos_read(
        push(0x2a) + push(0x1e) + OP_MSTORE8 + OP_MSIZE + push(0) + OP_SSTORE, "deadbeef");
    const auto header = dvmone::read_valid_eof1_header(bytes_view(code).begin());
    const auto analysis = analyze(DVMC_SHANGHAI, {&code[header.code_begin()], header.code_size});
