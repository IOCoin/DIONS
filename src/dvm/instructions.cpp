...
{
    return state.exit(DVMC_SUCCESS);
}
...
const instruction* op_sstore(const instruction* instr, AdvancedExecutionState& state) noexcept
{
    ...
    if ((state.gas_left -= gas_left_correction) < 0)
        return state.exit(DVMC_OUT_OF_GAS);
...
{
    return state.exit(DVMC_INVALID_INSTRUCTION);
}
...
constexpr std::array<instruction_exec_fn, 256> instruction_implementations = []() noexcept {
    ...
    table[OP_CREATE] = op_create<DVMC_CREATE>;
    table[OP_CALL] = op_call<DVMC_CALL>;
    table[OP_CALLCODE] = op_call<DVMC_CALLCODE>;
    table[OP_RETURN] = op_return<DVMC_SUCCESS>;
    table[OP_DELEGATECALL] = op_call<DVMC_DELEGATECALL>;
    table[OP_CREATE2] = op_create<DVMC_CREATE2>;
    table[OP_STATICCALL] = op_call<DVMC_CALL, true>;
    table[OP_REVERT] = op_return<DVMC_REVERT>;
...
constexpr op_table op_tables[] = {
    create_op_table<DVMC_FRONTIER>(),
    create_op_table<DVMC_HOMESTEAD>(),
    create_op_table<DVMC_TANGERINE_WHISTLE>(),
    create_op_table<DVMC_SPURIOUS_DRAGON>(),
    create_op_table<DVMC_BYZANTIUM>(),
    create_op_table<DVMC_CONSTANTINOPLE>(),
    create_op_table<DVMC_PETERSBURG>(),
    create_op_table<DVMC_ISTANBUL>(),
    create_op_table<DVMC_BERLIN>(),
    create_op_table<DVMC_LONDON>(),
};
static_assert(
    std::size(op_tables) == DVMC_MAX_REVISION + 1, "op table entry missing for an DVMC revision");
}  // namespace

DVMC_EXPORT const op_table& get_op_table(evmc_revision rev) noexcept
{
