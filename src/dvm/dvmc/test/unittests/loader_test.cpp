...
protected:
    ...
    static void setup(const char* path, const char* symbol, dvmc_create_fn fn) noexcept
    {
        dvmc_test_library_path = path;
        dvmc_test_library_symbol = symbol;
        dvmc_test_create_fn = fn;
    }

    static void destroy(dvmc_vm* /*vm*/) noexcept { ++destroy_count; }

    static dvmc_set_option_result set_option(dvmc_vm* /*vm*/,
                                             const char* name,
...
#pragma GCC diagnostic ignored "-Wconversion"
            return static_cast<dvmc_set_option_result>(-42);
#pragma GCC diagnostic pop
    ...
    static dvmc_vm* create_vm_barebone()
    {
        static auto instance =
            dvmc_vm{DVMC_ABI_VERSION, "vm_barebone", "", destroy, nullptr, nullptr, nullptr};
        ++create_count;
        return &instance;
    }

    /// Creates a VM mock with ABI version different than in this project.
    static dvmc_vm* create_vm_with_wrong_abi()
    {
        constexpr auto wrong_abi_version = 1985;
        static_assert(wrong_abi_version != DVMC_ABI_VERSION);
        static auto instance =
            dvmc_vm{wrong_abi_version, "", "", destroy, nullptr, nullptr, nullptr};
        ++create_count;
        return &instance;
    }

    /// Creates a VM mock with optional set_option() method.
    static dvmc_vm* create_vm_with_set_option() noexcept
    {
        static auto instance = dvmc_vm{
            DVMC_ABI_VERSION, "vm_with_set_option", "", destroy, nullptr, nullptr, set_option};
        ++create_count;
        return &instance;
    }
};
...
TEST_F(loader, load_nonexistent)
{
    ...
    EXPECT_TRUE(dvmc_load(path, &ec) == nullptr);
    EXPECT_EQ(ec, DVMC_LOADER_CANNOT_OPEN);
    EXPECT_TRUE(dvmc_load(path, nullptr) == nullptr);
}
...
TEST_F(loader, load_long_path)
{
    ...
    EXPECT_EQ(ec, DVMC_LOADER_INVALID_ARGUMENT);
    EXPECT_TRUE(dvmc_load(path.c_str(), nullptr) == nullptr);
    EXPECT_STREQ(dvmc_last_error_msg(),
                 "invalid argument: file name is too long (5000, maximum allowed length is 4096)");
    EXPECT_TRUE(dvmc_last_error_msg() == nullptr);
}
...
{
    dvmc_loader_error_code ec = DVMC_LOADER_UNSPECIFIED_ERROR;
    EXPECT_TRUE(dvmc_load(nullptr, &ec) == nullptr);
    EXPECT_EQ(ec, DVMC_LOADER_INVALID_ARGUMENT);
    EXPECT_STREQ(dvmc_last_error_msg(), "invalid argument: file name cannot be null");
    EXPECT_TRUE(dvmc_last_error_msg() == nullptr);
    EXPECT_TRUE(dvmc_load(nullptr, nullptr) == nullptr);
    EXPECT_STREQ(dvmc_last_error_msg(), "invalid argument: file name cannot be null");
    EXPECT_TRUE(dvmc_last_error_msg() == nullptr);
}
...
{
    dvmc_loader_error_code ec = DVMC_LOADER_UNSPECIFIED_ERROR;
    EXPECT_TRUE(dvmc_load("", &ec) == nullptr);
    EXPECT_STREQ(dvmc_last_error_msg(), "invalid argument: file name cannot be empty");
    EXPECT_TRUE(dvmc_last_error_msg() == nullptr);
    EXPECT_EQ(ec, DVMC_LOADER_INVALID_ARGUMENT);
    EXPECT_TRUE(dvmc_load("", nullptr) == nullptr);
    EXPECT_STREQ(dvmc_last_error_msg(), "invalid argument: file name cannot be empty");
    EXPECT_TRUE(dvmc_last_error_msg() == nullptr);
}
...
TEST_F(loader, load_aaa)
{
    ...
    for (auto& path : paths)
    {
        setup(path, "dvmc_create_aaa", create_aaa);
        dvmc_loader_error_code ec = DVMC_LOADER_UNSPECIFIED_ERROR;
        const auto fn = dvmc_load(path, &ec);
...
TEST_F(loader, load_windows_path)
{
    ...
    for (auto& path : paths)
    {
        ...
        {
            EXPECT_EQ(ec, DVMC_LOADER_CANNOT_OPEN);
            EXPECT_STREQ(dvmc_last_error_msg(), "cannot load library");
            EXPECT_TRUE(dvmc_last_error_msg() == nullptr);
        }
    }
}
...
TEST_F(loader, load_symbol_not_found)
{
    auto paths = {
        ...
        "_",
        "lib_.so",
        "unittests/double-prefix-aaa.dvm",
        "unittests/double_prefix_aaa.dvm",
    };
...
{
    setup("default.dvmc", "dvmc_create", create_aaa);
    ...
    fn = dvmc_load(dvmc_test_library_path, nullptr);
    EXPECT_EQ(fn, &create_aaa);
}

TEST_F(loader, load_and_create_failure)
{
    setup("failure.vm", "dvmc_create", create_failure);

    dvmc_loader_error_code ec = DVMC_LOADER_UNSPECIFIED_ERROR;
    auto vm = dvmc_load_and_create(dvmc_test_library_path, &ec);
    EXPECT_TRUE(vm == nullptr);
    EXPECT_EQ(ec, DVMC_LOADER_VM_CREATION_FAILURE);
    EXPECT_STREQ(dvmc_last_error_msg(), "creating DVMC VM of failure.vm has failed");
    EXPECT_TRUE(dvmc_last_error_msg() == nullptr);

    vm = dvmc_load_and_create(dvmc_test_library_path, nullptr);
    EXPECT_TRUE(vm == nullptr);
    EXPECT_STREQ(dvmc_last_error_msg(), "creating DVMC VM of failure.vm has failed");
}
...
{
    setup("abi1985.vm", "dvmc_create", create_vm_with_wrong_abi);
...
{
    setup("path", "dvmc_create", create_vm_with_set_option);

    dvmc_loader_error_code ec = DVMC_LOADER_UNSPECIFIED_ERROR;
    auto vm = dvmc_load_and_configure("path", &ec);
    EXPECT_TRUE(vm);
    ...
    vm = dvmc_load_and_configure("path,", &ec);
    EXPECT_TRUE(vm);
    EXPECT_TRUE(recorded_options.empty());
    EXPECT_EQ(ec, DVMC_LOADER_SUCCESS);
}
...
TEST_F(loader, load_and_configure_single_option)
{
    ...
    dvmc_loader_error_code ec = DVMC_LOADER_UNSPECIFIED_ERROR;
    auto vm = dvmc_load_and_configure("path,o=1", &ec);
    EXPECT_TRUE(vm);
    ...
    recorded_options.clear();
    vm = dvmc_load_and_configure("path,O=2", &ec);
    EXPECT_TRUE(vm);
    ASSERT_EQ(recorded_options.size(), size_t{1});
    EXPECT_EQ(recorded_options[0].first, "O");
...
TEST_F(loader, load_and_configure_uknown_option)
{
    ...
    dvmc_loader_error_code ec = DVMC_LOADER_UNSPECIFIED_ERROR;
    auto vm = dvmc_load_and_configure("path,z=1", &ec);
    EXPECT_FALSE(vm);
    ASSERT_EQ(recorded_options.size(), size_t{1});
    EXPECT_EQ(recorded_options[0].first, "z");
    EXPECT_EQ(recorded_options[0].second, "1");
    EXPECT_EQ(ec, DVMC_LOADER_INVALID_OPTION_NAME);
    EXPECT_STREQ(dvmc_last_error_msg(), "vm_with_set_option (path): unknown option 'z'");
    EXPECT_EQ(destroy_count, create_count);

    recorded_options.clear();
    vm = dvmc_load_and_configure("path,x=2,", &ec);
    EXPECT_FALSE(vm);
    ASSERT_EQ(recorded_options.size(), size_t{1});
    EXPECT_EQ(recorded_options[0].first, "x");
    EXPECT_EQ(recorded_options[0].second, "2");
    EXPECT_EQ(ec, DVMC_LOADER_INVALID_OPTION_VALUE);
    EXPECT_STREQ(dvmc_last_error_msg(),
...
TEST_F(loader, load_and_configure_multiple_options)
{
    ...
    dvmc_loader_error_code ec = DVMC_LOADER_UNSPECIFIED_ERROR;
    auto vm = dvmc_load_and_configure("path,a=_a,b=_b1,c=_c,b=_b2", &ec);
    EXPECT_TRUE(vm);
    ...
    recorded_options.clear();
    vm = dvmc_load_and_configure("path,a=_a,b=_b2,a=_c,", &ec);
    EXPECT_TRUE(vm);
...
TEST_F(loader, load_and_configure_uknown_option_in_sequence)
{
    ...
    dvmc_loader_error_code ec = DVMC_LOADER_UNSPECIFIED_ERROR;
    auto vm = dvmc_load_and_configure("path,a=_a,b=_b,c=_b,", &ec);
    EXPECT_FALSE(vm);
    ASSERT_EQ(recorded_options.size(), size_t{3});
    EXPECT_EQ(recorded_options[0].first, "a");
    EXPECT_EQ(recorded_options[0].second, "_a");
    ...
    recorded_options.clear();
    vm = dvmc_load_and_configure("path,a=_a,x=_b,c=_c", &ec);
    EXPECT_FALSE(vm);
    ...
    EXPECT_EQ(recorded_options[1].second, "_b");
    EXPECT_EQ(ec, DVMC_LOADER_INVALID_OPTION_NAME);
    EXPECT_STREQ(dvmc_last_error_msg(), "vm_with_set_option (path): unknown option 'x'");
    EXPECT_EQ(destroy_count, create_count);
}
...
TEST_F(loader, load_and_configure_empty_values)
{
    ...
    dvmc_loader_error_code ec = DVMC_LOADER_UNSPECIFIED_ERROR;
    auto vm = dvmc_load_and_configure("path,flag,e=,flag=,e", &ec);
    EXPECT_TRUE(vm);
    ASSERT_EQ(recorded_options.size(), size_t{4});
...
TEST_F(loader, load_and_configure_degenerated_names)
{
    ...
    dvmc_loader_error_code ec = DVMC_LOADER_UNSPECIFIED_ERROR;
    auto vm = dvmc_load_and_configure("path,,,=,,=xxx", &ec);
    EXPECT_TRUE(vm);
...
TEST_F(loader, load_and_configure_comma_at_the_end)
{
    ...
    dvmc_loader_error_code ec = DVMC_LOADER_UNSPECIFIED_ERROR;
    auto vm = dvmc_load_and_configure("path,x=x,", &ec);
    EXPECT_TRUE(vm);
    ASSERT_EQ(recorded_options.size(), size_t{1});
    EXPECT_EQ(recorded_options[0].first, "x");
...
TEST_F(loader, load_and_configure_vm_without_set_option)
{
    ...
    dvmc_loader_error_code ec = DVMC_LOADER_UNSPECIFIED_ERROR;
    auto vm = dvmc_load_and_configure("path,a=0,b=1", &ec);
    EXPECT_FALSE(vm);
    EXPECT_TRUE(recorded_options.empty());
    EXPECT_EQ(ec, DVMC_LOADER_INVALID_OPTION_NAME);
    EXPECT_STREQ(dvmc_last_error_msg(), "vm_barebone (path) does not support any options");
    EXPECT_EQ(destroy_count, create_count);
    ...
    EXPECT_TRUE(recorded_options.empty());
    EXPECT_EQ(ec, DVMC_LOADER_SUCCESS);
    EXPECT_FALSE(dvmc_last_error_msg());
    dvmc_destroy(vm);
    EXPECT_EQ(destroy_count, create_count);
...
TEST_F(loader, load_and_configure_error_not_wanted)
{
    ...
    auto vm = dvmc_load_and_configure("path,f=1", nullptr);
    EXPECT_FALSE(vm);
    ASSERT_EQ(recorded_options.size(), size_t{1});
    EXPECT_EQ(recorded_options[0].first, "f");
    EXPECT_EQ(recorded_options[0].second, "1");
    EXPECT_EQ(destroy_count, create_count);
    EXPECT_STREQ(dvmc_last_error_msg(), "vm_with_set_option (path): unknown option 'f'");
    EXPECT_FALSE(dvmc_last_error_msg());
}
