...
// TODO: This one is specifically for DVMereum, but we can make it generic in due course.
// TODO: hidden-partition style key-store.
/**
 * @brief High-level manager of password-encrypted keys for DVMereum.
 * Usage:
 *
 * Call exists() to check whdvmer there is already a database. If so, get the master password from
...
public:
	...
	static boost::filesystem::path defaultPath() { return getDataDir("dvmereum") / boost::filesystem::path("keys.info"); }
