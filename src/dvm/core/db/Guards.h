 ...
 * DVM_(m) d = 1;
 * ...
 * DVM_(m) { for (auto d = 10; d > 0; --d) foo(d); d = 0; }
 * @endcode
 *
...
#define DEV_GUARDED(MUTEX) \
	for (GenericGuardBool<Guard, Mutex> __dvm_l(MUTEX); __dvm_l.b; __dvm_l.b = false)
#define DEV_READ_GUARDED(MUTEX) \
	for (GenericGuardBool<ReadGuard, SharedMutex> __dvm_l(MUTEX); __dvm_l.b; __dvm_l.b = false)
#define DEV_WRITE_GUARDED(MUTEX) \
	for (GenericGuardBool<WriteGuard, SharedMutex> __dvm_l(MUTEX); __dvm_l.b; __dvm_l.b = false)
#define DEV_RECURSIVE_GUARDED(MUTEX) \
	for (GenericGuardBool<RecursiveGuard, RecursiveMutex> __dvm_l(MUTEX); __dvm_l.b; __dvm_l.b = false)
#define DEV_UNGUARDED(MUTEX) \
	for (GenericUnguardBool<Mutex> __dvm_l(MUTEX); __dvm_l.b; __dvm_l.b = false)
#define DEV_READ_UNGUARDED(MUTEX) \
	for (GenericUnguardSharedBool<SharedMutex> __dvm_l(MUTEX); __dvm_l.b; __dvm_l.b = false)
#define DEV_WRITE_UNGUARDED(MUTEX) \
	for (GenericUnguardBool<SharedMutex> __dvm_l(MUTEX); __dvm_l.b; __dvm_l.b = false)
