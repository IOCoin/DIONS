...
/*enum Check
{
    ...
    CheckParent,
    CheckValidation,
    CheckValidationQuickly,
    CheckAll = CheckBasic | CheckExtended | CheckBlock | CheckParent | CheckValidation,
};
 ...
 * from some given RLP block serialisation with the static fromHeader(), through the mdvmod
 ...
 * constructor BlockHeader(bytesConstRef) and manually with the populate() mdvmod. These will
 ...
 * populateFromParent() mdvmod. The genesis block info may be retrieved with genesis() and the
 * corresponding RLP block created with createGenesisBlock().
 *
 * To determine the header hash without the nonce (for validationing), the mdvmod hash(WithoutNonce) is
 * provided.
 *
...
public:
    ...
    h256 hash(IncludeValidation _i = WithValidation) const;
    void streamRLP(RLPStream& _s, IncludeValidation _i = WithValidation) const;
    ...
    template <class T> void setValidation(unsigned _offset, T const& _value) { Guard l(m_validationLock); if (m_validation.size() <= _offset) m_validation.resize(_offset + 1); m_validation[_offset] = rlp(_value); noteDirty(); }
    template <class T> void setValidation(T const& _value) { setValidation(0, _value); }
    ...
    template <class T> T validation(unsigned _offset = 0) const { T ret; Guard l(m_validationLock); if (_offset < m_validation.size()) ret = RLP(m_validation[_offset]).convert<T>(RLP::VeryStrict); return ret; }

private:
    void populate(RLP const& _header);
    void streamRLPFields(RLPStream& _s) const;
    std::vector<bytes> validation() const
    {
        Guard l(m_validationLock);
        return m_validation;
    }
    ...
    std::vector<bytes> m_validation;		///< Additional (RLP-encoded) header fields.
    mutable Mutex m_validationLock;

    mutable h256 m_hash;			///< (Memoised) SHA3 hash of the block header with validation.
    mutable h256 m_hashWithout;		///< (Memoised) SHA3 hash of the block header without validation.
...
{
    _out << _bi.hash(WithoutValidation) << " " << _bi.parentHash() << " " << _bi.sha3Uncles() << " " << _bi.author() << " " << _bi.stateRoot() << " " << _bi.transactionsRoot() << " " <<
