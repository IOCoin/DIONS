...
/// Named-boolean type to encode whdvmer a signature be included in the serialisation process.
...
public:
    ...
    /// @returns the amount of DVM to be transferred by this (message-call) transaction, in Wei. Synonym for endowment().
    u256 value() const { return m_value; }

    /// @returns the base fee and thus the implied exchange rate of DVM to GAS.
    ...
    int64_t baseGasRequired(DVMSchedule const& _es) const { return baseGasRequired(isCreation(), &m_data, _es); }

    /// Get the fee associated for a transaction with the given data.
    static int64_t baseGasRequired(bool _contractCreation, bytesConstRef _data, DVMSchedule const& _es);
...
protected:
    ...
    u256 m_value;						///< The amount of DVM to be transferred by this transaction. Called 'endowment' for contract-creation transactions.
    Address m_receiveAddress;			///< The receiving address of the transaction.
    u256 m_gasPrice;					///< The base fee and thus the implied exchange rate of DVM to GAS.
    ...
    /// https://github.com/dvmereum/EIPs/blob/master/EIPS/eip-155.md
