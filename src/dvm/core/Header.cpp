...
BlockHeader::BlockHeader(BlockHeader const& _other) :
    ...
    m_validation(_other.validation()),
...
BlockHeader& BlockHeader::operator=(BlockHeader const& _other)
{
    ...
    m_author = _other.author();
    m_difficulty = _other.difficulty();
    std::vector<bytes> validation = _other.validation();
    {
...
h256 BlockHeader::hash(IncludeValidation _i) const
{
    h256 dummy;
    Guard l(m_hashLock);
    h256& memo = _i == WithValidation ? m_hash : _i == WithoutValidation ? m_hashWithout : dummy;
    if (!memo)
    {
        RLPStream s;
        streamRLP(s, _i);
...
void BlockHeader::streamRLP(RLPStream& _s, IncludeValidation _i) const
{
    if (_i != OnlyValidation)
    {
        _s.appendList(BlockHeader::BasicFields + (_i == WithoutValidation ? 0 : m_validation.size()));
        BlockHeader::streamRLPFields(_s);
    }
    if (_i != WithoutValidation)
        for (unsigned i = 0; i < m_validation.size(); ++i)
            _s.appendRaw(m_validation[i]);
}
...
void BlockHeader::populate(RLP const& _header)
{
    ...
    try
    {
        ...
        for (unsigned i = 13; i < _header.itemCount(); ++i)
            m_validation.push_back(_header[i].data().toBytes());
    }
