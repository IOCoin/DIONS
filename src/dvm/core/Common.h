...
static const u256 dvmer = exp10<18>();
...
struct ImportRequirements
{
	...
	enum
	{
		...
		CheckUncles = UncleBasic | UncleValidations, ///< Check uncle validations.
		CheckTransactions = TransactionBasic | TransactionSignatures, ///< Check transaction signatures.
		OutOfOrderChecks = ValidValidation | CheckUncles | CheckTransactions, ///< Do all checks that can be done independently of prior blocks having been imported.
		InOrderChecks = Parent | UncleParent, ///< Do all checks that cannot be done independently of prior blocks having been imported.
		Everything = ValidValidation | CheckUncles | CheckTransactions | Parent | UncleParent,
		None = 0
	};
};
...
// TODO: move back into a mining subsystem and have it be accessible from Validationant only via a dynamic_cast.
/**
