...
void NoProof::init()
{
    DVM_REGISTER_VALIDATION_ENGINE(NoProof);
}

void NoReward::init()
{
    DVM_REGISTER_VALIDATION_ENGINE(NoReward);
}
...
void NoProof::populateFromParent(BlockHeader& _bi, BlockHeader const& _parent) const
{
    ...
    _bi.setDifficulty(calculateDVMashDifficulty(chainParams(), _bi, _parent));
    _bi.setGasLimit(calculateGasLimit(chainParams(), _bi));
}

void NoProof::generateValidation(BlockHeader const& _bi)
{
    BlockHeader header(_bi);
    header.setValidation(NonceField, h64{0});
    header.setValidation(MixHashField, h256{0});
    RLPStream ret;
...
void NoProof::verify(Strictness _s, BlockHeader const& _bi, BlockHeader const& _parent, bytesConstRef _block) const
{
    ...
    if (_parent)
    {
        ...
        auto expected = calculateDVMashDifficulty(chainParams(), _bi, _parent);
...
void ValidationEngineFace::verify(Strictness _s, BlockHeader const& _bi, BlockHeader const& _parent, bytesConstRef _block) const
{
...
void ValidationEngineFace::populateFromParent(BlockHeader& _bi, BlockHeader const& _parent) const
{
    _bi.populateFromParent(_parent);
}

void ValidationEngineFace::verifyTransaction(ImportRequirements::value _ir, TransactionBase const& _t,
                                       BlockHeader const& _header, u256 const& _gasUsed) const
{
    ...
    dvm::DVMSchedule const& schedule = dvmSchedule(_header.number());
...
ValidationEngineFace* ValidationEngineRegistrar::create(ChainOperationParams const& _params)
{
    ValidationEngineFace* ret = create(_params.validationEngineName);
    assert(ret && "Validation engine not found.");
    if (ret)
        ret->setChainParams(_params);
    return ret;
}

DVMSchedule const& ValidationEngineBase::dvmSchedule(u256 const& _blockNumber) const
{
    return chainParams().scheduleForBlockNumber(_blockNumber);
}

u256 ValidationEngineBase::blockReward(u256 const& _blockNumber) const
{
    DVMSchedule const& schedule{dvmSchedule(_blockNumber)};
