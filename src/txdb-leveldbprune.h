...
#ifndef BITCOIN_LEVELDB_H_
#define BITCOIN_LEVELDB_H_
...
private:
    leveldb::DB *pdbprune;  // Points to the global instance.
...
protected:
    ...
    bool Read(const K& key, T& value)
    {
        ...
        if (readFromDb) {
            leveldb::Status status = pdbprune->Get(leveldb::ReadOptions(),
                                              ssKey.str(), &strValue);
            if (!status.ok()) {
    ...
    bool Write(const K& key, const T& value)
    {
        ...
        leveldb::Status status = pdbprune->Put(leveldb::WriteOptions(), ssKey.str(), ssValue.str());
        if (!status.ok()) {
    ...
    bool Erase(const K& key)
    {
        ...
        leveldb::Status status = pdbprune->Delete(leveldb::WriteOptions(), ssKey.str());
    ...
    bool Exists(const K& key)
    {
        ...
        leveldb::Status status = pdbprune->Get(leveldb::ReadOptions(), ssKey.str(), &unused);
